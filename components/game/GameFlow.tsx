...
const Wrap = ({ children, wide }: { children: React.ReactNode; wide?: boolean }) => (
...
export const GameFlow = ({ onBack, onMovil }: { onBack: () => void; onMovil: () => void }) => {
  ...
  const handleSelectNivel = (nivel: NivelUso) => {
    ...
    const newDev: DeviceAdded = {
      uid, id: nivelModal.id, nivel,
      mbps: nivelModal.mbps[nivel],
      name: nivelModal.name, emoji: nivelModal.emoji, color: nivelModal.color,
    };
...
// Fix: query paralela por operador para evitar que Movistar (173k filas)
//      monopolice el limit(2000) y deje fuera a Tigo/ETB/Claro.
// ─────────────────────────────────────────────────────────────────────────────

const OPERADORES_FASE1 = ["Claro", "Movistar", "Etb", "Tigo"];
const TIPOS_HOGAR      = ["internet", "paquete", "tv"];
const PLANES_POR_OP    = 500; // suficiente para cubrir los ~20-43 únicos por operador
...
const calcularYBuscar = async () => {
  ...
  try {
    // ── Query paralela: una por operador ─────────────────────────────────────
    // Movistar tiene 173k filas → un limit global de 2000 casi todo lo llena.
    // Con queries paralelas cada operador aporta hasta PLANES_POR_OP filas,
    // luego scorarPlanes() deduplica por operador+nombre internamente.
    ...
    // Combinar todos los resultados, ignorar errores individuales
    const rawData: any[] = [];
    results.forEach((r, i) => {
      if (r.error) {
        console.error(`Error consultando ${OPERADORES_FASE1[i]}:`, r.error);
      } else {
        rawData.push(...(r.data ?? []));
      }
    });

    console.log(`📦 Total filas recibidas: ${rawData.length} (antes de deduplicar)`);

    const planes = scorarPlanes(rawData, res, 3);
    setPlanesDB(planes);

    console.log(`✅ Planes finales: ${planes.length}`, planes.map(p => `${p.operador} ${p._score}pts`));
    ...
    <Wrap>
      <div style={{ textAlign: "center", padding: "30px 0" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 14, flexWrap: "wrap", marginBottom: 36 }}>
          {[["🎯","Perfil","Tu avatar digital"],["🏠","Casa","Dispositivos + uso"],["📊","Consumo","Mbps reales"],["🤖","Plan IA","Recomendación"]].map(([ic, n, d]) => (
        ...
        {AVATARS.map((av) => {
          ...
          return (
            <div key={av.id} onClick={() => {
              ...
              {sel && av.id !== "nomada" && <div style={{ marginTop: 7, color: av.color, fontSize: 10, fontWeight: 800 }}>✓ Seleccionado</div>}
            </div>
          );
        })}
      </div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ color: C.muted, fontSize: 12, fontWeight: 700, marginBottom: 10, textAlign: "center" }}>¿Cuántas personas usan internet simultáneamente?</div>
          ...
          {(Object.entries(PERSONAS_CONFIG) as [PersonasHogar, typeof PERSONAS_CONFIG[PersonasHogar]][]).map(([key, cfg]) => {
            ...
            return (
              <div key={key} onClick={() => setPersonas(key)} style={{
                ...
                border: `2px solid ${sel ? C.neon : C.borderSoft}`, borderRadius: 12, transition: "all .2s",
              }}>
    ...
    <div style={{ minHeight: "100vh", background: BG, color: "#fff", fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div className="gameflow-split" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "100vh" }}>
        <div style={{ padding: "90px 16px 24px", display: "flex", flexDirection: "column", borderRight: `1px solid ${C.borderSoft}` }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${C.borderSoft}`, borderRadius: 11, padding: "10px 13px", marginTop: 10 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 99, height: 8, overflow: "hidden", marginBottom: 6 }}>
              <div style={{ height: "100%", borderRadius: 99, width: `${Math.min((mbpsTotal / 300) * 100, 100)}%`, background: `linear-gradient(90deg,${mbpsTotal < 50 ? "#10b981" : mbpsTotal < 150 ? "#f59e0b" : "#ef4444"},#00d4ff)`, transition: "width .4s" }} />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
              <div><span style={{ color: C.muted }}>Suma base: </span><span style={{ color: C.neon, fontWeight: 800 }}>{mbpsTotal} Mbps</span></div>
              <div><span style={{ color: C.muted }}>Recomendado: </span><span style={{ color: C.yellow, fontWeight: 800 }}>{mbpsRec} Mbps</span></div>
            </div>
          </div>
          {devices.length > 0 && (
            <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 5 }}>
              {devices.map((d) => {
                ...
                return (
                  <div key={d.uid} onClick={() => remDev(d.uid)} title="Clic para quitar" style={{ cursor: "pointer", background: `${d.color}14`, border: `1px solid ${d.color}33`, borderRadius: 7, padding: "3px 8px", display: "flex", alignItems: "center", gap: 4 }}>
        ...
        <div style={{ padding: "90px 16px 24px", overflowY: "auto" }}>
          ...
          <p style={{ color: C.muted, fontSize: 11, textAlign: "center", marginBottom: 14 }}>Al agregar cada dispositivo te preguntaremos cómo lo usas — eso define el consumo real</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9, marginBottom: 18 }}>
            {DEVICES.map((d) => {
              const cnt = devices.filter((x) => x.id === d.id).length;
              return (
                <div key={d.id} onClick={() => handleClickDevice(d)} style={{
                  ...
                  <div style={{ color: d.color, fontSize: 10, fontWeight: 700 }}>{d.mbps.bajo}–{d.mbps.alto} Mbps</div>
        ...
        <>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            <h2 style={{ fontWeight: 900, fontSize: "clamp(1.3rem,4vw,1.8rem)", marginBottom: 8, color: "#fff" }}>
              Tu hogar necesita <span style={{ color: resumen.colorConsumo }}>{resumen.mbpsRecomendado} Mbps</span>
            </h2>
            <p style={{ color: C.muted, fontSize: 13, maxWidth: 480, margin: "0 auto" }}>{generarExplicacion(resumen)}</p>
          </div>
          <div style={{ background: "rgba(0,212,255,0.04)", border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
            ...
            <div style={{ borderTop: `1px solid ${C.borderSoft}`, marginTop: 10, paddingTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, textAlign: "center" }}>
              {[["Suma base", `${resumen.mbpsBase} Mbps`, C.neon], ["× Personas", `×${resumen.factorSimultaneidad}`, C.yellow], ["× Perfil", `×${resumen.factorAvatar}`, C.neon2]].map(([l, v, c]: any) => (
          ...
          <div style={{
            ...
            {planesDB.map((p) => (
              <Card key={p.id_crc} glow={p.glow} style={{ padding: 18, position: "relative", border: p.top ? `2px solid ${p.glow}` : undefined }}>
                {p.top && (
                  <div style={{ position: "absolute", top: -1, right: 16, background: p.glow, color: "#000", fontSize: 9, fontWeight: 900, padding: "3px 10px", borderRadius: "0 0 8px 8px" }}>RECOMENDADO</div>
                )}
