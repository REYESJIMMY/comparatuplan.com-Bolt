...
import { Check, Search, Zap, Shield, BookOpen, ShoppingCart, ArrowRight } from "lucide-react";
...
export const Hero = ({ onGame, onQuiz }: HeroProps) => {
    ...
    <section style={{ position: "relative", borderRadius: 20, overflow: "hidden", border: `1px solid ${C.border}`, background: "rgba(6,4,22,0.7)", padding: "clamp(28px,5vw,48px) clamp(16px,4vw,32px) 40px", minHeight: 330 }}>
      ...
      <div style={{ position: "relative", zIndex: 2 }}>
        ...
        <div style={{ background: "rgba(0,212,255,0.04)", border: `1px solid ${C.border}`, borderRadius: 11, padding: "12px 15px", marginBottom: 20, maxWidth: 400 }}>
          ...
          {([[mayor, setMayor, "Soy mayor de edad (18+)"], [habea, setHabea, "Acepto Política de Tratamiento de Datos"]] as [boolean, (v: boolean) => void, string][]).map(([val, set, label], i) => (
            <label key={i} style={{ display: "flex", alignItems: "center", gap: 9, cursor: "pointer", marginBottom: i === 0 ? 7 : 0 }}>
              <div
                onClick={() => set(!val)}
                style={{ width: 15, height: 15, borderRadius: 4, border: `2px solid ${val ? C.neon : "rgba(255,255,255,0.15)"}`, background: val ? C.neon : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s", flexShrink: 0 }}
              >
                {val && <Check size={8} color="#04040f" strokeWidth={3} />}
              </div>
        ...
        <div className="hero-actions" style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-start" }}>
          <div>
            <GlowBtn onClick={onQuiz} disabled={!ready} gradient="linear-gradient(135deg,#0070cc,#0050aa)" glow={C.neon} style={{ borderRadius: 11, padding: "11px 24px", fontSize: 14 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
                <Search size={14} />Comparar Planes<ArrowRight size={12} />
              </span>
            </GlowBtn>
...
export const FeaturedPlans = ({ onQuiz, addToCart }: { onQuiz: () => void; addToCart: (item: any) => void }) => (
  <section>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
      ...
      <GlowBtn onClick={onQuiz} gradient="linear-gradient(135deg,#0070cc,#0050aa)" glow={C.neon} style={{ padding: "7px 14px", fontSize: 11, borderRadius: 9 }}>Ver todos →</GlowBtn>
    </div>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 11 }}>
            <button
              ...
              style={{ flex: 1, background: `${p.glow}12`, border: `1px solid ${p.glow}28`, borderRadius: 8, padding: "7px 0", color: p.glow, fontWeight: 700, fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, transition: "all .15s" }}
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {[
          ...
          { emoji: "📱", title: "Planes Móviles", desc: "Datos ilimitados", color: C.neon2, action: onQuiz },
          { emoji: "📺", title: "TV + Streaming", desc: "Combos desde $89.900", color: C.cyan, action: onQuiz },
